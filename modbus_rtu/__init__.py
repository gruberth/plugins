...
AttrFactor = 'modBusRtuFactor'
AttrByteOrder = 'modBusRtuByteOrder'
AttrWordOrder = 'modBusRtuWordOrder'
AttrSlaveUnit = 'modBusRtuUnit'
AttrObjectType = 'modBusRtuObjectType'
AttrDirection = 'modBusRtuDirection'


class modbus_rtu(SmartPlugin):
    ...
    def __init__(self, sh, *args, **kwargs):
        ...
        self.logger.info('Init modbus_rtu plugin')

        # Call init code of parent class (SmartPlugin)
        super().__init__()

        self._tty = self.get_parameter_value('tty')
        self._baud = self.get_parameter_value('baud')
        self._parity = self.get_parameter_value('parity')
        self._stopbits = int(self.get_parameter_value('stopbits'))
        self._cycle = int(self.get_parameter_value('cycle'))
        self._ignoreList = eval(self.get_parameter_value('ignoreList')[0])
        ...
        if self._parity not in ['None', 'Even', 'Odd']:
    ...
    def parse_item(self, item):
        ...
        if self.has_iattr(item.conf, AttrAddress):
            ...
            if self.has_iattr(item.conf, AttrSlaveUnit):
                ...
                if slaveUnit in self._ignoreList:
                    self.logger.info("Ignoring item: {0}.".format(item, ))
                    return
    ...
    def __write_Registers(self, regPara, value):
        ...
        if dataType.lower() == 'uint':
            if bits == 16:
                ...
                self.logger.error("Number of bits {0} not supported : {1}".format(bits, dataTypeStr))
            ...
            self.logger.error("Datatype {0} not supported : {1}".format(dataType.lower(), dataTypeStr))
            return None
